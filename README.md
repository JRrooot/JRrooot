## 👋 About Me
Cybersecurity enthusiast focused on penetration testing and offensive security. Currently learning ethical hacking through hands-on labs and real-world PoCs. Interested in web application security, vulnerability research, and exploit development.

---

## 🛡️ Certifications
- CPTS (in progress)
- Certified Ethical Hacker (CEH)
- CompTIA Security+

---

## 🛠️ Projects
- **[Hash Cracker](https://github.com/JRrooot/hash-cracker)**  
  A Python-based hash cracking tool supporting common hashing algorithms using wordlists.

- **[Webmin 1.580 RCE PoC (CVE-2012-2982)](https://github.com/JRrooot/CVE-2012-2982-Webmin-RCE)**  
  Python proof-of-concept exploit demonstrating authenticated RCE in Webmin 1.580.

---

## 💻 Skills
- **Vulnerability Assessment & Penetration Testing (VAPT)**
- **Programming & Scripting:** Python, Bash, PowerShell, C++, Assembly  
- **Networking**
- **Web Application Security**
- **Linux Administration**

---

## 📫 Contact
- 📧 Email: j.jaffinronald@gmail.com
